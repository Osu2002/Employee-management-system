<template>
    <div :class="'select2-' + id + '-wrapper col-6 mb-4'">
        <label class="form-label" :for="id" v-if="label">{{ label }}
            <sup class="text-danger" v-if="isRequired">*</sup></label>
        <select class="form-select select2" :id="id" :multiple="isMultiple" v-model="model" :disabled="isDisabled"
            :required="isRequired" @change="handleChange">
            <option value="">-- Select --</option>
            <option :value="option.id" v-for="option in options" :key="option.id" v-bind:selected="option.id == model">
                {{ option.name }}
            </option>
        </select>
